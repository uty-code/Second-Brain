...
- **텍스트 라벨 (Labels)**: 
  ...
  - **AI Gaze Tracking 하이라이트 (예외적 글로우 허용)**: 
    - AI 에이전트가 탐색(순회) 중인 활성 노드는 예외적으로 **에메랄드 그린 컬러 (`#10B981` / Tailwind `emerald-500`)** 테두리와 미세한 에메랄드 글로우(Glow Ring), 초록색 텍스트 라벨을 3초 동안 유지합니다. (이는 일반 야광 효과 금지 조항의 유일한 시각적 예외로, AI의 실시간 관심 추적(Gaze Tracking) 인지를 위해 강제됩니다.)
...
- **계정 및 세션 관리 UI (LNB 하단 Footer)**:
  - 좌측 사이드바(LNB) 최하단에는 현재 로그인한 사용자 아이디(`currentUser`)와 아바타 버튼을 고정 배치합니다.
  - 아바타 버튼을 클릭하면 위로 팝업되는 드롭다운 메뉴를 통해 '로그아웃' 및 '계정 탈퇴' 작업을 선명하게 분리하여 노출합니다.
  - **계정 탈퇴 확인 모달**: '계정 탈퇴' 선택 시 생성되는 모달은 파괴적인 최종 결정을 시각적으로 안내하기 위해 붉은 경고 아이콘(AlertTriangle)과 `text-red-400` 계열의 경고 문구를 포함하며, 실수 방지를 위해 "취소" 버튼과 최종 파괴 버튼을 대조 배치합니다.
