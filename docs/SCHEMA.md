---
title: "Database & Graph Schema Specification"
type: reference
...
erDiagram
    ...
    WorkspaceCredentials {
        ...
        VARCHAR notion_api_key
        VARCHAR github_api_key
        VARCHAR deepseek_api_key
    ...
    Users ||--o| Workspace : "default"
    Workspace ||--o| WorkspaceCredentials : "has"
    Workspace ||--o{ RawSource : "owns"
    Workspace ||--o{ WikiPage : "owns"
    Workspace ||--o{ OutboxEvent : "logs"
```

### 1.2 `Users` 테이블 (B2B 사용자 계정)
...
### 1.4 `WorkspaceCredentials` 테이블 (BYOK 암호화 API 자격증명)
...
### 1.6 `WikiPage` 테이블 (마크다운 위키 페이지 메타데이터)
- **`id`** (`BIGINT`, `IDENTITY(1,1)`, `PRIMARY KEY`): 고유 식별자
- **`workspace_id`** (`VARCHAR(50)`, `INDEX`): 소속 워크스페이스 ID
...
### 1.7 `OutboxEvent` 테이블 (Transactional Outbox)
...
### 2.1 온톨로지 클래스 관계도 (Ontology Diagram)
...
classDiagram
    ...
    Concept --> Source : DERIVED_FROM
```

### 2.2 노드 레이블 및 속성 (Node Labels & Properties)
...
### 2.3 관계 유형 및 속성 (Relationships & Properties)
...
### 2.4 제약 조건 및 인덱스 (Constraints & Indexes)
```cypher
...
### 2.5 그래프 갱신 예제 Cypher 쿼리 (MyBatis/Java 연동)
```cypher
