# AIMS-Graph 공통 규칙 및 안전 가드레일 (project-rules.md)

이 파일은 AIMS-Graph 프로젝트에 기여하는 모든 AI 에이전트가 공통으로 반드시 준수해야 하는 상위 규칙과 가드레일(Hooks)을 정의합니다.

---

## 1. CRITICAL: 안전 가드레일 (Hooks)

### LLM Wiki Guard (NO RAG)
...
### TDD Guard (테스트 우선 개발)
- 기능 구현 파일 또는 컴포넌트를 수정하거나 새로 생성할 때, 해당 기능에 대응하는 테스트 파일이 반드시 함께 존재하거나 변경되어야 한다.
- 테스트 없이 구현 코드만 작성하는 것은 절대 금지.
- 테스트가 먼저 실패(Red)하는 것을 확인한 후, 구현을 작성하여 통과(Green)시킬 것.

### Dangerous Command Guard (위험 명령 제한)
...
### Phase Completion Guard (작업 완료 가드)
- Phase 완료 시 `tasks.md` 갱신과 더불어, 반드시 작업한 `phases/phase-*.md` 최상단에 `> **Status: [완료됨]**` 및 핵심 결정사항을 `- **히스토리**:`로 간결히 기록할 것. (유지보수 단계의 단순 태스크는 `docs/updates.md` 내에 완료 표시로 대체)
