...
public class IngestionWorkerTest {
  ...
  @Mock private RLock indexLock;

  @Mock private RLock logLock;

  @Mock private LlmService llmService;

  @Mock private com.aimsgraph.domain.wiki.WikiService wikiService;
  ...
  void setUp() {
    ingestionWorker =
        new IngestionWorker(objectMapper, redissonClient, neo4jClient, llmService, wikiService);
  }
  ...
  void testProcessEvent() throws Exception {
    ...
    when(llmService.extractKnowledge(anyString(), anyString(), anyString()))
        .thenReturn(List.of(mockConcept));
