...
public class WorkspaceController {
  ...
  public ResponseEntity<Map<String, Object>> createWorkspace(@RequestBody Map<String, String> body)
    ...
    if (name == null || name.isBlank()) {
      return ResponseEntity.badRequest()
          .body(Map.of("error", "INVALID_NAME", "message", "Workspace name is required"));
    }
      ...
      return ResponseEntity.status(409)
          .body(
              Map.of(
                  ...
                  "ALREADY_EXISTS",
                  "message",
                  "Workspace '" + sanitized + "' already exists"));
    }
  ...
  public ResponseEntity<Map<String, Object>> getWorkspaceGraph(
    ...
    if (!workspaceId.equals(currentTenant)) {
      return ResponseEntity.status(403)
          .body(Map.of("error", "FORBIDDEN", "message", "Workspace mismatch"));
    }
            ...
            .map(
                row ->
                    Map.of(
                        ...
                        row.get("id"),
                        "name",
                        row.get("name") != null ? row.get("name") : row.get("id"),
                        "type",
                        "concept",
                        "val",
                        1))
  ...
  public void exportWorkspace(
      @PathVariable("workspace_id") String workspaceId, HttpServletResponse response)
      throws IOException {
    ...
    try (ZipOutputStream zos = new ZipOutputStream(response.getOutputStream())) {
      ...
      var results =
          neo4jClient
              .query(
                  "MATCH (n {workspaceId: $workspaceId})-[r]-(m {workspaceId: $workspaceId}) RETURN n, r, m")
              .bind(workspaceId)
              .to("workspaceId")
              .fetch()
              .all();
  ...
  public ResponseEntity<Map<String, Object>> deleteWorkspaceData(
      @PathVariable("workspace_id") String workspaceId) throws IOException {
    ...
    if (Files.exists(workspacePath)) {
      ...
      java.util.List<Path> filesToDelete =
  ...
  public ResponseEntity<Map<String, Boolean>> getCredentialsStatus(
    ...
    boolean hasNotion =
        creds != null && creds.getNotionApiKey() != null && !creds.getNotionApiKey().isBlank();
    boolean hasGithub =
        creds != null && creds.getGithubApiKey() != null && !creds.getGithubApiKey().isBlank();
    boolean hasDeepseek =
        creds != null && creds.getDeepseekApiKey() != null && !creds.getDeepseekApiKey().isBlank();
  ...
  public ResponseEntity<Map<String, String>> updateCredentials(
      @PathVariable("workspace_id") String workspaceId, @RequestBody Map<String, String> body) {
    String currentTenant =
        (String)
            RequestContextHolder.currentRequestAttributes()
                .getAttribute(
