...
public class AnalyzeController {
  ...
  /** 여러 파일을 업로드하면 OpenAI Responses API를 통해 지식을 추출하고 통합된 그래프 데이터(nodes + links)로 반환합니다. */
  ...
  public ResponseEntity<Map<String, Object>> analyzeFiles(
    ...
    if (files.length == 0) {
      return ResponseEntity.badRequest().body(Map.of("error", "NO_FILES", "message", "파일이 없습니다."));
    }
    ...
    Map<String, Object> graphData =
        llmService.analyzeFilesWithOpenAI(files, workspaceId, modelName);
      ...
      return ResponseEntity.badRequest()
          .body(
              Map.of(
                  ...
                  "NO_TEXT",
                  "message",
                  "No text could be extracted from the provided Notion page."));
    }
    Map<String, Object> graphData =
        llmService.analyzeTextWithOpenAI(rawText, workspaceId, modelName);
    return ResponseEntity.ok(graphData);
  }
