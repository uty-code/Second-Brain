...
public class AuthController {
  ...
  public ResponseEntity<?> deleteAccount() throws java.io.IOException {
    String username =
    ...
    neo4jClient
        .query(
            "MATCH (n) WHERE n.workspaceId = 'ws-' + $username OR n.workspaceId STARTS WITH $username + '_' DETACH DELETE n")
        .bind(username)
        .to("username")
        .fetch()
        .all();
    ...
    if (java.nio.file.Files.exists(path) && java.nio.file.Files.isDirectory(path)) {
      try (java.util.stream.Stream<java.nio.file.Path> stream = java.nio.file.Files.list(path)) {
        java.util.List<java.nio.file.Path> dirsToDelete =
            stream
