  ...
  protected void doFilterInternal(
    ...
    if (token != null) {
      if ("MVP_DUMMY_TOKEN".equals(token)) {
        ...
        String workspaceId =
            (headerWorkspaceId != null && !headerWorkspaceId.isEmpty())
                ? headerWorkspaceId
                : "default-workspace";

        UsernamePasswordAuthenticationToken auth =
            new UsernamePasswordAuthenticationToken("dummy_user", null, Collections.emptyList());
        SecurityContextHolder.getContext().setAuthentication(auth);

        if (RequestContextHolder.getRequestAttributes() != null) {
          RequestContextHolder.getRequestAttributes()
              .setAttribute(
                  JwtInterceptor.WORKSPACE_ID_ATTRIBUTE,
                  workspaceId,
