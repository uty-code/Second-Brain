...
- **Run ID:** `merge_2026-09-17_20-58-35`
- **Evaluated At:** 2026-09-17T21:00:11.051331900
- **Model:** `gpt-4o-mini`
- **Total Scenarios:** 5
...
| `case_c_temporal` | `temporal_change` | ✅ PASS | Java 21 Current, Forbidden: 0 | 최신 상태 사실([Java 21, 가상 스레드])이 현재 운영 상태로 정상 반영됨 |
...
| `case_e_bloat` | `bloat_prevention` | ✅ PASS | Init: 837 -> Final: 2495 | 10단계 연속 병합 후 최종 문서 길이(2495자)가 단순 Append 폭발(4,000~8,000자) 없이 통제 상한선(2500자) 이내로 압축 유지됨 |
...
**Evaluation Metrics:**
```json
{expansionRatio=1.43, mergedLength=1076, existingLength=751, missingRequiredFacts=[], duplicateRatio=0.0}
```

**Evaluation Reasons & Proofs:**
- 동일/유사 문장 중복 비율(0.0)이 자동화 회귀 임계값(0.15) 이하로 안정적임
- 문서 팽창 배율(1.4327563249001332)이 상한선(1.6) 이하로 통제됨
...
- 과거 레거시 이력([스레드 풀])이 역사적 맥락으로 온전히 보존됨
...
**Evaluation Metrics:**
```json
{perspectiveMatches={strong_consistency=[강한 일관성, ACID, 원자성, 2PC], high_availability=[가용성, 최종 일관성, Saga]}, missingRequiredFacts=[]}
```
...
**Evaluation Metrics:**
```json
{finalLength=2495, initialLength=837, missingRequiredFacts=[], duplicateRatio=0.0, lengthTrajectory=[837, 1000, 1155, 1313, 1486, 1657, 1852, 2012, 2181, 2352, 2495]}
```

**Evaluation Reasons & Proofs:**
- 10단계 연속 병합 후 최종 문서 길이(2495자)가 단순 Append 폭발(4,000~8,000자) 없이 통제 상한선(2500자) 이내로 압축 유지됨
