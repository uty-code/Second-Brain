...
title: 백엔드 동시성 아키텍처
summary: 2026년 6월 기준 AIMS 백엔드는 Java 21로 업그레이드하고 가상 스레드를 채택했습니다.
type: architecture
...
## 동작 원리
요청이 인입되면 가상 스레드가 할당되어 작업을 수행하며, 블로킹 I/O 호출 발생 시 캐리어 스레드는 언마운트(unmount)되어 다른 가상 스레드를 실행하므로 스레드 낭비가 전혀 없습니다. 이전 모델에서는 풀에서 유휴 스레드를 할당받아 작업을 수행하며, 풀이 가득 차면 작업 큐에 적재되었습니다.

## 장점과 트레이드오프
- 하드웨어 리소스 효율을 극대화하고 처리량을 대폭 향상시킵니다.
- OS 레벨에서 검증된 스레드 모델로 안정적입니다.
- I/O 블로킹 시 스레드가 점유되어 메모리 낭비 및 컨텍스트 스위칭 오버헤드가 발생할 수 있습니다.
- synchronized 블록 사용 시 캐리어 스레드가 피닝(pinning)될 수 있어 ReentrantLock으로 대체해야 하는 주의점이 있습니다.

## 실전 적용 사례
2026-06-01부터 프로덕션 전체 서버에 Virtual Threads 모델이 기본 적용되었습니다. 이전에는 모든 REST API 및 DB 트랜잭션 요청 처리에 Thread Pool 모델이 적용되었습니다.
