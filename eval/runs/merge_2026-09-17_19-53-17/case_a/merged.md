...
## 정의
Apache Kafka는 분산 커밋 로그(distributed commit log) 구조를 핵심으로 하는 고성능 메시징 인프라입니다. 다수의 서버 노드인 브로커들이 클러스터를 이루어 동작하며, 대규모 이벤트 스트리밍을 지원합니다.

## 핵심 구성 요소
- **브로커(Broker)**: 메시지를 디스크에 영구 저장하고 클라이언트의 읽기/쓰기 요청을 중계하는 카프카 서버 노드입니다. 클라이언트의 레코드 송수신을 담당하며 분산 파일 시스템에 로그를 기록합니다.
