...
- **Run ID:** `merge_2026-09-17_19-56-31`
- **Evaluated At:** 2026-09-17T19:57:57.315131500
- **Model:** `gpt-4o-mini`
- **Total Scenarios:** 5
...
| `case_e_bloat` | `bloat_prevention` | ✅ PASS | Init: 837 -> Final: 2414 | 10단계 연속 병합 후 최종 문서 길이(2414자)가 단순 Append 폭발(4,000~8,000자) 없이 통제 상한선(2500자) 이내로 압축 유지됨 |
...
**Evaluation Metrics:**
```json
{expansionRatio=1.43, mergedLength=1075, existingLength=751, missingRequiredFacts=[], duplicateRatio=0.0}
```

**Evaluation Reasons & Proofs:**
- 동일/유사 문장 중복 비율(0.0)이 자동화 회귀 임계값(0.15) 이하로 안정적임
- 문서 팽창 배율(1.4314247669773634)이 상한선(1.6) 이하로 통제됨
...
**Evaluation Metrics:**
```json
{finalLength=2414, initialLength=837, missingRequiredFacts=[], duplicateRatio=0.0, lengthTrajectory=[837, 981, 1104, 1276, 1431, 1602, 1797, 1933, 2100, 2271, 2414]}
```

**Evaluation Reasons & Proofs:**
- 10단계 연속 병합 후 최종 문서 길이(2414자)가 단순 Append 폭발(4,000~8,000자) 없이 통제 상한선(2500자) 이내로 압축 유지됨
