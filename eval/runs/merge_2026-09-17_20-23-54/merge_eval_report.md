...
- **Run ID:** `merge_2026-09-17_20-23-54`
- **Evaluated At:** 2026-09-17T20:25:19.281536500
- **Model:** `gpt-4o-mini`
- **Total Scenarios:** 5
...
| `case_e_bloat` | `bloat_prevention` | ✅ PASS | Init: 837 -> Final: 2385 | 10단계 연속 병합 후 최종 문서 길이(2385자)가 단순 Append 폭발(4,000~8,000자) 없이 통제 상한선(2500자) 이내로 압축 유지됨 |
...
**Evaluation Metrics:**
```json
{expansionRatio=1.43, mergedLength=1074, existingLength=751, missingRequiredFacts=[], duplicateRatio=0.0}
```

**Evaluation Reasons & Proofs:**
- 동일/유사 문장 중복 비율(0.0)이 자동화 회귀 임계값(0.15) 이하로 안정적임
- 문서 팽창 배율(1.430093209054594)이 상한선(1.6) 이하로 통제됨
...
**Evaluation Metrics:**
```json
{finalLength=2385, initialLength=837, missingRequiredFacts=[], duplicateRatio=0.0, lengthTrajectory=[837, 971, 1094, 1229, 1402, 1573, 1768, 1904, 2071, 2242, 2385]}
```

**Evaluation Reasons & Proofs:**
- 10단계 연속 병합 후 최종 문서 길이(2385자)가 단순 Append 폭발(4,000~8,000자) 없이 통제 상한선(2500자) 이내로 압축 유지됨
